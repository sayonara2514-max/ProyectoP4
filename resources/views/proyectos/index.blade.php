    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mt-4">
        <table class="w-full text-sm">
            ...
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="p-3">{{ $p->programa->nombre }}</td>
                <td class="p-3">$ {{ number_format($p->presupuesto,2) }}</td>
                <td class="p-3">
                ...
                <td class="p-3 space-x-1">
                    <a href="{{ route('proyectos.show', $p) }}" class="text-blue-600 text-xs">Ver</a>

                    @if(in_array(auth()->user()->rol?->nombre, ['Administrador','Técnico de Planificación']) && $p->estado === 'formulado')
                        <a href="{{ route('proyectos.edit', $p) }}" class="text-yellow-600 text-xs">Editar</a>
                        <form action="{{ route('proyectos.enviarRevision', $p) }}" method="POST" class="inline">@csrf<button class="text-purple-600 text-xs">Enviar a Revision</button></form>
                    @endif

                    @if(in_array(auth()->user()->rol?->nombre, ['Administrador','Revisor Institucional']) && $p->estado === 'en_revision')
                        <form action="{{ route('proyectos.validar', $p) }}" method="POST" class="inline">@csrf<button class="text-green-600 text-xs">Validar</button></form>
                        <button onclick="document.getElementById('modal-proyecto-{{ $p->id }}').classList.remove('hidden')" class="text-orange-600 text-xs">Devolver</button>
                    @endif

                    @if(in_array(auth()->user()->rol?->nombre, ['Administrador','Autoridad Validante']) && $p->estado === 'validado')
                        <form action="{{ route('proyectos.aprobar', $p) }}" method="POST" class="inline">@csrf<button class="text-green-700 text-xs font-bold">Aprobar</button></form>
                        <button onclick="document.getElementById('modal-proyecto-{{ $p->id }}').classList.remove('hidden')" class="text-orange-600 text-xs">Devolver</button>
                    @endif

                    @if(auth()->user()->rol?->nombre === 'Administrador' && $p->estado === 'formulado')
                        <form action="{{ route('proyectos.destroy', $p) }}" method="POST" class="inline" onsubmit="return confirm('Eliminar?')">@csrf @method('DELETE')<button class="text-red-600 text-xs">Eliminar</button></form>
                    @endif
                </td>
            </tr>
                ...
                <div class="bg-white rounded-lg p-6 w-96">
                    ...
                    <form action="{{ route('proyectos.devolver', $p) }}" method="POST">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" onclick="document.getElementById('modal-proyecto-{{ $p->id }}').classList.add('hidden')" class="bg-gray-200 px-4 py-2 rounded">Cancelar</button>
                        </div>
                    </form>
                </div>
            </div>
