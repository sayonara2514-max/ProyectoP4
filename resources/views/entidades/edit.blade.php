...
<div class="py-6 max-w-xl mx-auto px-4">
    <form method="POST" action="{{ route('entidades.update', $entidad->id) }}" class="space-y-4">
        ...
        <div><label class="block text-sm font-medium">Misión</label><textarea name="mision" class="w-full border rounded p-2">{{ old('mision', $entidad->mision) }}</textarea></div>
        <div><label class="block text-sm font-medium">Estructura</label><textarea name="estructura" class="w-full border rounded p-2">{{ old('estructura', $entidad->estructura) }}</textarea></div>
